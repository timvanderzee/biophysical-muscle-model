...
%     odeopt = [];
    ...
    Q1 = x0(2);
    Fce   = Q1 + Q0;
    Fce0  = log(1+exp(Fce*parms.K))/parms.K;    
    F0 = Fce0 + parms.Fpe0;
    ...
    sol0 = ode15s(@(t,y) fiber_dynamics_explicit_no_tendon(t,y, parms), [0 1], x0, odeopt);
...
%     end
    ...
    Li      = interp1(sol.x, sol.y(4,:), toc); % length
    ...
    Ldi  = interp1(sol.x, xdot(4,:), toc); % second-order moment time derivative
    ...
    [error_Q0i, error_Q1i, error_Q2i, F0dot, Rdot] = MuscleEquilibrium(Q0i, Q1i, pi, qi, dQ0dti, dQ1dti, dQ2dti, parms.f, parms.w, parms.k11, parms.k12, parms.k21, parms.k22,  Noni, Ldi, DRXi, parms.b, parms.k, Ri, parms.dLcrit, parms.ps2, parms.approx); % cross-bridge dynamics
    error_Ri = dRdti - Rdot;
    ...
    %%
    Ftot = Fi + parms.Fpe_func(Li, parms);
    
    dLse =  log(Ftot./parms.kse0 + 1) / parms.kse;
    Lce = Lts - dLse;
