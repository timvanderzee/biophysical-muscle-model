...
if approx
    ...
    IGef{1} = @(c,k)(c(1,:)*k(1).*exp(m*tanh((c(3,:)*k(2)^2/4-c(2,:)*k(2))/m)));
    IGef{2} = @(c,k)(c(1,:)*k(1).*exp(m*tanh((c(3,:)*k(2)^2/4-c(2,:)*k(2))/m))).*(c(2,:)-c(3,:)*k(2)/2);
    IGef{3} = @(c,k)(c(1,:)*k(1).*exp(m*tanh((c(3,:)*k(2)^2/4-c(2,:)*k(2))/m))).*((c(2,:)-c(3,:)*k(2)/2).^2+c(3,:)/2);
    ...
    % erf function (forcible detachment)
